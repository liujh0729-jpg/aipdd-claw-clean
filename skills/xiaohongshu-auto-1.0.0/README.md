...
本项目也支持作为 WorkBuddy 的目录型 Skill 使用。请复制完整仓库（不能只复制 `SKILL.md`），并将目录命名为 `post-to-xhs`。

WorkBuddy 的任务工作区与 Skill 安装目录可能不同；调用脚本时应先定位 `SKILL.md` 所在目录，再使用脚本绝对路径。任务产生的标题、正文、素材和导出文件应留在任务工作区。

```bash
python "$SKILL_DIR/scripts/cdp_publish.py" check-login
```

安装、扫码登录、远程 CDP 与发布示例详见 [docs/workbuddy-integration.md](docs/workbuddy-integration.md)。
