> **EN:** WeChat MP layout component library for the Moyu Ticket theme (ticket/receipt style).
>
# 公众号排版组件库 —— 摸鱼票据风
...
**签名文案适配**：MODULE.md 的作者签名（"我是 {{作者名}}…"两段，默认占位、由用户替换）以正文段落（组件 5）形式放在本组件**之前**；本组件内部 `{{互动文案}}` 直接使用 MODULE.md 固定的第二段（"如果你觉得今天这篇有收获…三连，我们下篇见"），不要重复堆叠两句"三连"。
...
<section style="max-width:677px;margin:0 auto;background:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;color:#374151;line-height:1.75;letter-spacing:0.5px;">
  ...
  <!-- 6. 固定签名段落（组件5 正文段落，MODULE.md 固定文案，footer-cta 之前） -->
