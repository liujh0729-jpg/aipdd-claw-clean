...
---
> **EN:** WeChat Official Account article layout engine: Markdown → paste-ready HTML with theme library, auto chapter numbering, keyword underlines, quote cards, TOC, code blocks, images/GIFs, author signature; supports Markdown/Word/PDF/plain text, one-click auto layout, and custom theme generation.
>
...
核心资产是 `references/` 下的**主题组件库**（每套一个主题：设计变量 + 各组件完整 HTML + 模板骨架 + 映射规则）外加 1 套**通用增量库**（代码块 / 图片·GIF / 小标签标题，所有主题共用）。主题清单以 `references/theme-index.md` 为单一来源。本 MODULE.md 只负责流程与决策，**具体 HTML 代码一律从组件库取，不要凭记忆手写**。
