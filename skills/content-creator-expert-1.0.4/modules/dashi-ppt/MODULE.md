...
---
> **EN:** PPT deck creation: generates offline-openable, browser-editable HTML presentations from preset visual theme combinations; exports PPTX/PDF.
>
...
当前 `MODULE.md` 所在目录就是 Skill 根目录,下文记为 `<skill-root>`。

**项目模板位置（重要）**：由于包大小限制，本精简版未内置 `project/` 运行时目录。首次使用时按以下顺序查找 project 模板：

1. 环境变量 `DASHI_PPT_PROJECT_PATH` 指定的路径（如果已设置）
2. 与 `content-creator-expert` 同级的原始 dashi-ppt 目录：`<skill-root>/../../../dashi-ppt-skill-main/skills/dashi-ppt/project`
3. 上述位置存在时，将其记为 `<project-root>`；如果都不存在，请告知用户需要完整的 dashi-ppt skill 才能运行 PPT 生成

下文所有引用 `<skill-root>/project` 的地方，都替换为 `<project-root>`。
...
- 风格选择提问:用户可见回复必须用列表形式列出当前可选风格（见下方theme-choice-hints）和极简“适合/人群”供用户选择;不可写入 `goal.json` 或任何 media 字段。
